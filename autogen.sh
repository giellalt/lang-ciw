...
function print_usage() {
    ...
    echo "  -l, --add-langvar   Add env. variable \$GTLANG_ciw to login script."
...
if test x$langvar = xlangvar ; then
	if test x$LOGINFILE = x ; then
		...
		if test x$GTLANG_ciw = x$LANGDIR ; then
			echo "\$GTLANG_ciw already defined."
		else
			# Already defined with a different value:
			if test x$GTLANG_ciw != x ; then
				renew=renew
				OLDLANGDIR=$GTLANG_ciw
			fi

			# Add the variable to the login script:
			cp $LOGINFILE $LOGINFILE.gtbackup.${DATE}-${TIME}
			echo "export GTLANG_ciw=$LANGDIR" >> $LOGINFILE
			source $LOGINFILE

			# Feedback depending on whether it was added or redefined:
			if test x$renew = xrenew ; then
			 echo "The env. variable \$GTLANG_ciw has been redefined in"
			 echo "$LOGINFILE. The old value $OLDLANGDIR is no longer in use."
			else
			 echo "The env. variable \$GTLANG_ciw has been added to $LOGINFILE."
...
# the current dir:
if   test x$GTLANG_ciw = x ; then
	echo "WARNING: The variable \$GTLANG_ciw has not been defined. You will not"
	...
	echo "$0 -l"
	echo
elif test x$GTLANG_ciw != x$LANGDIR ; then
	echo "WARNING: The variable \$GTLANG_ciw has the value:"
	echo "  $GTLANG_ciw"
